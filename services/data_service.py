    ...
    def get_gold_data(self, timeframe='H1', limit=1000):
        ...
        if self.use_mt5_csv and self.mt5_csv_reader:
            try:
                ...
                # Increase default limit for better Price Action analysis
                enhanced_limit = max(limit, 500)  # At least 500 candles
                df = self.mt5_csv_reader.get_gold_data(timeframe, enhanced_limit)
...
class DataService:
    ...
    def get_market_data(self, timeframe='H1', limit=1000):
        """Get market data using the main service"""
        return self.service.get_gold_data(timeframe=timeframe, limit=limit)
        
    def get_gold_data(self, timeframe='H1', limit=1000):
        """Get gold data using the main service"""
        return self.service.get_gold_data(timeframe=timeframe, limit=limit)
