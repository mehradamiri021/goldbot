...
class TelegramService:
    ...
    async def send_daily_report(self, analysis_data):
        ...
        try:
            ...
            telegram_msg.message_type_name = 'REPORT'
            telegram_msg.recipient_type = 'CHANNEL'
            telegram_msg.recipient_id = self.channel_id
            telegram_msg.message_text = report
            telegram_msg.status = 'SENT' if message else 'FAILED'
            telegram_msg.telegram_message_id = str(message.message_id) if message else None
            telegram_msg.sent_at = datetime.utcnow() if message else None
    ...
    def send_message_to_channel(self, message):
        """Send message to channel (sync version for compatibility)"""
        try:
            import requests
    ...
    async def send_message_to_channel_async(self, message):
        """Send message to channel (async version)"""
        try:
            result = await self.send_message(self.channel_id, message)
            return result is not None
        except Exception as e:
            logger.error(f"❌ Failed to send async message to channel: {e}")
            return False
    ...
    async def send_admin_report(self, analysis_data):
        ...
        try:
            ...
            telegram_msg = TelegramMessage()
            telegram_msg.message_type_name = 'ADMIN'
            telegram_msg.recipient_type = 'ADMIN'
            telegram_msg.recipient_id = self.admin_id
            telegram_msg.message_text = report
            telegram_msg.status = 'SENT' if message else 'FAILED'
            telegram_msg.telegram_message_id = str(message.message_id) if message else None
            telegram_msg.sent_at = datetime.utcnow() if message else None
    ...
    def send_admin_notification_sync(self, message, urgent=False):
    ...
    async def send_signal_to_channel(self, signal_data):
        ...
        try:
            ...
            telegram_msg = TelegramMessage()
            telegram_msg.message_type_name = 'SIGNAL'
            telegram_msg.recipient_type = 'CHANNEL'
            telegram_msg.recipient_id = self.channel_id
            telegram_msg.message_text = signal_text
            telegram_msg.status = 'SENT' if message else 'FAILED'
            telegram_msg.telegram_message_id = str(message.message_id) if message else None
            telegram_msg.sent_at = datetime.utcnow() if message else None
            db.session.add(telegram_msg)
...
✅ سیستم به طور کامل راه‌اندازی و آماده کار است.
            ...
            channel_log.message_type_name = 'STARTUP_TEST'
            channel_log.recipient_type = 'CHANNEL'
            channel_log.recipient_id = self.channel_id
            channel_log.message_text = message_text
            channel_log.status = 'SENT' if channel_message else 'FAILED'
            channel_log.telegram_message_id = str(channel_message.message_id) if channel_message else None
            channel_log.sent_at = datetime.utcnow() if channel_message else None
            db.session.add(channel_log)
            
            # Log admin message
            admin_log = TelegramMessage()
            admin_log.message_type_name = 'STARTUP_STATUS'
            admin_log.recipient_type = 'ADMIN'
            admin_log.recipient_id = self.admin_id
            admin_log.message_text = admin_message
            admin_log.status = 'SENT' if admin_message_obj else 'FAILED'
            admin_log.telegram_message_id = str(admin_message_obj.message_id) if admin_message_obj else None
            admin_log.sent_at = datetime.utcnow() if admin_message_obj else None
    ...
    async def send_admin_notification(self, message_text: str, message_type: str = 'NOTIFICATION'):
        ...
        try:
            ...
            telegram_msg = TelegramMessage()
            telegram_msg.message_type_name = message_type
            telegram_msg.recipient_type = 'ADMIN'
            telegram_msg.recipient_id = self.admin_id
