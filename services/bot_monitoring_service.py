...
from services.admin_notification_service_fixed import send_admin_notification
...
class BotMonitoringService:
    ...
    def update_component_status(self, component: str, status: str, error_message: str = None, additional_info: dict = None):
        ...
        try:
            ...
            # Update database - Fixed SQLAlchemy query
    ...
    def get_bot_health_report(self):
        ...
        try:
            ...
            # Get component statuses - Fixed SQLAlchemy queries
            from models import BotStatus, PriceAlert, Signal
            statuses = BotStatus.query.all()
            status_dict = {}
            
            for status in statuses:
                status_dict[status.component_name] = {
...
"""
            ...
            # Send to admin
            send_admin_notification(message, 'WARNING')
...
⚠️ حرکت قابل توجه در بازار طلا مشاهده شد
            ...
            # Send to admin
            send_admin_notification(message, 'INFO')
        ...
        try:
            ...
            for component in components:
                old_statuses = BotStatus.query.filter(
                    BotStatus.component_name == component
