...
export default function Dashboard() {
  ...
  const manualRefresh = async () => {
    await queryClient.resetQueries({ queryKey: ["/api/prices"] });
    await refetchPrices();
  };
  ...
  // دریافت آخرین قیمت‌ها - فقط با auto-refresh
  const { data: latestPrices, refetch: refetchPrices } = useQuery<PriceData>({
    queryKey: ["/api/prices"],
    refetchInterval: false, // غیرفعال کردن auto-refresh اتوماتیک
    staleTime: 0,
    gcTime: 0,
    refetchOnMount: true,
  ...
  const updatePricesMutation = useMutation({
    ...
    onSuccess: async (data) => {
      // بروزرسانی فوری فقط پس از دریافت داده‌های جدید
      await manualRefresh();
      
      toast({
        title: "قیمت‌ها بروزرسانی شد", 
        description: `شمش طلا: $${data?.data?.goldUSD || 'نامشخص'} | ${data?.sources?.navasan ? '✅ نوسان' : '❌ نوسان'} | ${data?.sources?.zaryaal ? '✅ تلگرام' : '❌ تلگرام'}`,
      });
    },
    onError: (error: any) => {
      toast({
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-slate-700/50 rounded">
                    ...
                    <span className="font-mono font-bold" data-testid="price-usd">
                      {latestPrices.usd > 0 ? formatPrice(latestPrices.usd) + ' تومان' : 'منبع API نوسان غیرفعال'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-3 bg-slate-700/50 rounded">
                    ...
                    <span className="font-mono font-bold" data-testid="price-eur">
                      {latestPrices.eur > 0 ? formatPrice(latestPrices.eur) + ' تومان' : 'منبع API نوسان غیرفعال'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-3 bg-slate-700/50 rounded">
                    ...
                    <span className="font-mono font-bold" data-testid="price-gold">
                      {latestPrices.gold18k > 0 ? formatPrice(latestPrices.gold18k) + ' تومان' : 'منبع API نوسان غیرفعال'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-3 bg-slate-700/50 rounded">
                    ...
                    <span className="font-mono font-bold" data-testid="price-coin-detail">
                      {latestPrices.coin > 0 ? formatPrice(latestPrices.coin) + ' تومان' : 'منبع API نوسان غیرفعال'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-3 bg-slate-700/50 rounded">
                    ...
                    <span className="font-mono font-bold" data-testid="price-bitcoin">
                      {latestPrices.bitcoin > 0 ? formatPrice(latestPrices.bitcoin) + ' تومان' : 'منبع API نوسان غیرفعال'}
                    </span>
                  </div>
                </div>
              ) : (
