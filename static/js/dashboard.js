...
// Setup auto-refresh with real-time price updates
function setupAutoRefresh() {
    ...
    // Check for price changes every 10 seconds
    setInterval(() => {
        checkPriceUpdates();
    }, 10 * 1000);
    ...
    setInterval(() => {
        ...
        const timestampElements = document.querySelectorAll('.last-update-time');
...
function checkPriceUpdates() {
    fetch('/api/live_status')
        ...
        .then(data => {
            ...
            // Update prices without full reload if they changed
            updatePricesQuickly(data);
        })
        .catch(error => {
...
// Quick price update without full page elements reload
function updatePricesQuickly(data) {
    // Update gold price
    const priceElement = document.getElementById('current-price');
    if (priceElement && data.current_price) {
    ...
    // Update USD prices
    const usdBuyElement = document.querySelector('.text-info');
    if (usdBuyElement && data.usd_buy) {
        const formattedPrice = data.usd_buy.toLocaleString('fa-IR');
        if (usdBuyElement.textContent !== formattedPrice) {
            usdBuyElement.textContent = formattedPrice;
            addUpdateAnimation(usdBuyElement);
        }
    }
    
    // Update 18k gold price
    const gold18kElement = document.querySelector('.text-success');
    if (gold18kElement && data.gold_18k) {
        const formattedPrice = data.gold_18k.toLocaleString('fa-IR');
        if (gold18kElement.textContent !== formattedPrice) {
            gold18kElement.textContent = formattedPrice;
            addUpdateAnimation(gold18kElement);
        }
    }
}
